...
This component provides emitters for logging the trace spans in text and/or JSON format using the `sf4j` logging framework for the JVM and `scalajs-logging` JS logger for the JavaScript (JS) platform.  It uses the `circe` library for formatting the trace span information as JSON.  It uses the `io.chrisdavenport.log4cats` library to abstract the particular platform targeted from the logging API.

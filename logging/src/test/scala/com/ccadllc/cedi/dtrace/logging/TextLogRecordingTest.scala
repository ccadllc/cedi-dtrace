...
class TextLogRecordingTest extends WordSpec with RecordingTestSupport {
  ...
  override protected def spanNote(note: Note) = s"name=${note.name},value=${note.value.fold("")(_.toString)}"

  "the distributed trace recording mechanism for emitting to a log with free text" should {

...
final class TraceT[F[_], A](private[dtrace] val tie: TraceContext[F] => F[A]) { self =>
   ...
   * @return newTraceT - a new instance of `TraceT` representing a child span of `this`.
   */
   ...
   * @param F - an instance of an `fs2.util.Catchable[F]` and `fs2.util.Suspendable[F]` in implicit scope.
   * @return effectfulProgramFA - the underlying effectful program `F[A]` that, when run, will calculate and record the trace of its execution.
   */
   ...
   * in implicit scope and the right-hand side represents the successful result.  It is a way of handling
   * non-fatal errors by turning them into `scala.util.Either` values.
   * @param F - an instance of `fs2.util.Catchable[F]` in implicit scope.
   * @return traceTFofThrowableOrA - a `TraceT[F, Either[Throwable, A]]` indicating that the underlying effectful program `F`
  ...
  /**
   * Recover from certain errors by mapping them to a `B` value.
   *
   * @param f - a partial function from `Throwable` to `B` which is passed the underlying `F[A]` program
   *   failure exception and which may handle it by returning a `B` value.
   * @param F - an instance of `fs2.util.Catchable[F]` in implicit scope.
   * @return traceTForFB - a new `TraceT[F, B]` with the error handling of the aforementioned `f` function
   *   parameter.
   * @see [[handleWith]] to recover from certain errors by mapping them to `TraceT[F, B]` values.
   * @see [[handleAllWith]] to recover from all errors by mapping them to `TraceT[F, B]` values.
   */
  ...
  /**
   * Recover from certain errors by mapping them to a `TraceT[F, B]` value.
   *
   * @param f - a partial function from `Throwable` to `TraceT[F, B]` which is passed the underlying `F[A]` program
   *   failure exception and which may handle it by returning a `TraceT[F, B]` value.
   * @param F - an instance of `fs2.util.Catchable[F]` in implicit scope.
   * @return traceTForFB - a new `TraceT[F, B]` with the error handling of the aforementioned `f` function
   *   parameter.
   * @see [[handle]] to recover from certain errors by mapping them to `B` values.
   * @see [[handleAllWith]] to recover from all errors by mapping them to `TraceT[F, B]` values.
   */
  ...
  /**
   * Recover from all errors by mapping them to a `TraceT[F, B]` value.
   *
   * @param f - a function from `Throwable` to `TraceT[F, B]` which is passed the underlying `F[A]` program
   *   failure exception and which handles it by returning a `TraceT[F, B]` value.
   * @param F - an instance of `fs2.util.Catchable[F]` in implicit scope.
   * @return traceTForFB - a new `TraceT[F, B]` with the error handling of the aforementioned `f` function
   *   parameter.
   * @see [[handle]] to recover from certain errors by mapping them to `B` values.
...
object TraceT extends TraceTPolyFunctions with TraceTInstances {
   ...
   * @param F - the `fs2.util.Suspendable[F]` in implicit scope.
   * @return traceTOfTA - a non-strict expression which computes `TraceT[F, A]` lifted into a `TraceT` in
...
private[dtrace] sealed trait TraceTInstancesLowPriority {
  ...
  /**
   * A `fs2.util.Effect[TraceT[F, ?]]` typeclass instance given an instance of `fs2.util.Effect[F]`
   * in implicit scope.  In other words, if a `TraceT`'s underlying effectful program `F` has
   * an implicit `fs2.util.Effect[F]` in scope, that `TraceT` will then have this in implicit scope.
   */
...
private[dtrace] sealed trait TraceTInstances extends TraceTInstancesLowPriority {
  /**
   * A `fs2.util.Async[TraceT[F, ?]]` typeclass instance given an instance of `fs2.util.Async[F]`
   * in implicit scope.  In other words, if a `TraceT`'s underlying effectful program `F` has
   * an implicit `fs2.util.Async[F]` in scope, that `TraceT` will then have this in implicit scope.
   */

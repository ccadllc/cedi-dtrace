...
class TestEmitterRecordingTest extends WordSpec with BeforeAndAfterEach with Matchers with TestData {
  private class TestEmitter extends TraceSystem.Emitter {
    ...
    override def emit[F[_]](tc: TraceContext)(implicit F: Async[F]): F[Unit] = {
      def formatText(context: TraceContext) = {
